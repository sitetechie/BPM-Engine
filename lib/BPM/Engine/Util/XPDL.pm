...
sub xml_doc {
    my $arg = shift
        or throw_param error => "Empty file, string or IO handle (xml_doc)";
...
sub xml_hash {
    my $arg = shift
        or throw_param error => "Empty file, string or IO handle (xml_hash)";

    my $xmldata = XML::LibXML::Simple::XMLin(
        $arg,
        ForceArray => [
            qw/
...
sub xpdl_doc {
    my $doc = shift
        or throw_param error => "Empty file, string or IO handle (xpdl_doc)";
    ...
    eval {
        ...
        my @nodes
            = $doc->documentElement->getElementsByTagName('XPDLVersion');
        ...
        if ( my $err = $@ ) {
            if ( ref($err) eq 'XML::LibXML::Error' ) {
                die "Non-conformant XML: "
                    . $err->message
                    . " in file "
                    . File::Basename::fileparse( $err->file )
                    . " (schema "
                    . File::Basename::fileparse($schema_file) . ")"
...
sub xpdl_hash {
    my $arg = shift
        or throw_param error => "Empty file, string or IO handle (xpdl_hash)";

    my $doc = xpdl_doc($arg);
    my $xpdl;
    eval { $xpdl = ( BPM::XPDL->from($doc) )[1]; };
    if ($@) {
        die "BPM::XPDL error: $@";
    }
...
sub _clean_bizagi_xml {
    ...
    _remove_attributes( $xmldoc, 'ConnectorGraphicsInfo', 'FromPort',
        'ToPort' );
...
sub _remove_elements {
    ...
    if ( $tag =~ /\// ) {
        my $xpath
            = '//xpdl:Package/xpdl:' . join( '/xpdl:', split( '\/', $tag ) );
...
sub _xpdl_spec {
    ...
    if ( $@ =~ /Failed to find shared file/ ) {
        ...
        $file = File::Spec->catpath( $volume, $directory,
            "../../../../share/schemas/$fname" );
        unless ( -e $file ) {
        ...
        unless ( -r $file ) {
            throw_install error =>
                "Schema '$fname' cannot be read, no read permissions";
        }
    }
    elsif ($@) {
        throw_install error => "Schema '$fname' not found in shared dirs: $@";
