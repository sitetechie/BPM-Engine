...
sub _create_from_hash {
    ...
    my $create_txn = sub {
        ...
        $entry->graph_conformance(
            $args->{ConformanceClass}->{GraphConformance} )
        ...
        _import_processes( $entry,
            $args->{WorkflowProcesses}->{WorkflowProcess} )
...
sub _import_application {
    ...
    my $app
        = $entry->result_source->schema->resultset('Application')->create(
        {   application_uid   => $args->{Id},
            application_name  => $args->{Name} || $args->{Id},
            parent_node       => $entry->id,
            description       => $args->{Description},
            application_scope => ref($entry) =~ /Package/
            ? 'Package'
            : 'Process',
        }
        );
...
sub _import_process {
    ...
    my $process = $entry->add_to_processes( { process_uid => $args->{Id} } );
...
sub _import_activity {
    ...
    $activity->start_mode('Manual')
        if (
        $args->{StartMode}
        && (ref( $args->{StartMode} )
            ? $args->{StartMode}->{Manual}
            : ( $args->{StartMode} eq 'Manual' ) )
        );
    $activity->finish_mode('Manual')
        if (
        $args->{FinishMode}
        && (ref( $args->{FinishMode} )
            ? $args->{FinishMode}->{Manual}
            : ( $args->{FinishMode} eq 'Manual' ) )
        );
        ...
        && $args->{TransitionRestrictions}->{TransitionRestriction} )
    {
        ...
        foreach my $r (@restrict) {
            ...
            foreach my $rtype (@rkeys) {
                ...
                my $pos = 1;
                foreach my $trans (
                    @{ $r->{$rtype}->{TransitionRefs}->{TransitionRef} } )
                {
    ...
    if ( $activity->is_implementation_type ) {
        ...
        my @itypes = keys %{$impl};
        die("Invalid Implementation specification")
            if ( scalar(@itypes) > 1 );
        ...
        if ( $activity->is_impl_task ) {
            ...
            && !$activity->is_impl_subflow
            && !$activity->is_impl_reference )
        {
            #die("Invalid Activity implementation");
        }
    }
    elsif (!$activity->is_route_type
        && !$activity->is_block_type
...
sub _prepare_task {
    ...
    if ( $type eq 'Application' ) {
        my $actual_params
            = delete $task->{ActualParameters}->{ActualParameter};
        if ($actual_params) {
    ...
    foreach my $msgtype ( keys %msgtypes ) {
        ...
        if ( $type =~ /$re/i ) {
            ...
            if ($params) {
                $msg->{ActualParameters}->{ActualParameter}
                    = _mapxml($params);
            }
            else {
...
sub _import_transition {
    ...
    my $from
        = $process->activities->search( { activity_uid => $act_out } )->next
    ...
    if ( $args->{Condition} ) {
        ...
        if ( ref($condition) eq 'XML::LibXML::Element' ) {
            ...
            $transition->condition_type( $args->{Condition}->{Type}
                    || 'NONE' );
        ...
        if ( $ctype eq 'EXCEPTION' ) {
            if ( my $dead
                = delete $deadline_map->{ $transition->condition_expr } )
            {
...
sub _set_elements {
    ...
    my $e
        = { extended_attr => [ 'ExtendedAttributes', 'ExtendedAttribute' ] };
...
#warn "Storing field $field multi $multi single $single type $type container $container entry $entry"
        ...
        if ( !$single && $multi eq 'Event' ) {
          ...
          # get rid of XML::LibXML::Element objects from mixed-schema elements
            if ( $multi eq 'ExtendedAttributes'
                && ref( $json->[0] ) eq 'XML::LibXML::Element' )
            {
                $json = [
                    map {
            ...
            elsif ($container eq 'ActivityTask'
                && $multi eq 'ActualParameters' )
            {
                $json = _mapxml($json);
            }
            elsif ( $multi
