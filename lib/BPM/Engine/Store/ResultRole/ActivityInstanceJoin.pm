...
sub _and_join_should_fire {
    ...
    my $activity = $self->activity;
    my $deferred_states
        = $self->result_source->schema->resultset('ActivityInstance')
        ->deferred(
...
sub _upstream_blocked {
    ...
    my $rs = $self->process_instance->activity_instances_rs(
        { tokenset => $self->tokenset, } )->active_or_completed;
    ...
    while ( my $next = shift(@act) ) {
        ...
        # active or completed+blocked instances
            ...
            # completed, is_split, blocked transition path
                ...
                # OR-split should be blocked, XOR split missed this transition
                # by definition
                if ( $upstream_act->is_or_split ) {
                    ...
                    die("OR split " . $upstream_act->activity_uid
                        . " completed but not blocked" )
                        unless $blocked;
                }
    ...
    die( "Invalid transition " . $transition->transition_uid )
        unless $block == $seen;
    return 1;
}
...
sub fire_join {
    ...
    $self->result_source->schema->resultset('ActivityInstance')->deferred(
        {   activity_id         => $self->activity->id,
            process_instance_id => $self->process_instance_id,
            tokenset            => $self->tokenset,
        }
...
sub _mark_upstream_joined {
    ...
    while ( $upstream_ai = $self->prev ) {
        ...
        if ( $upstream_ai->activity->is_or_split ) {
            ...
            # mark transition from split as 'fired' in join from this
            # downstream branch
