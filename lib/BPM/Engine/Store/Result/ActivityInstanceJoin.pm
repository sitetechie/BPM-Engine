...
package BPM::Engine::Store::Result::ActivityInstanceJoin;
BEGIN {
    $BPM::Engine::Store::Result::ActivityInstanceJoin::VERSION   = '0.001';
    $BPM::Engine::Store::Result::ActivityInstanceJoin::AUTHORITY = 'cpan:SITETECH';
    }
...
__PACKAGE__->set_primary_key(qw/ join_id /);

__PACKAGE__->belongs_to(
    activity_instance => 'BPM::Engine::Store::Result::ActivityInstance', 'token_id'
    );

# set transition in a join (when following transition from a split in ActivityRunner.pm)
sub set_transition {
    my ($self, $transition_id, $state) = @_;
    
    my $states = $self->states || {};
    if($states->{ $transition_id } && $state ne 'joined') {
        die("Transition state '$state' already set in Join as '" . $states->{$transition_id} . "'");
        }
