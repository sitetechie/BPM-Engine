...
sub _run {
    ...
    while ( my $act_ctx = $self->_queue_next ) {
        ...
        $acted{ $act_ctx->[0]->id }++;
        $self->debug(
            "runner: _run DrainInner " . $act_ctx->[0]->activity_uid );
    ...
    if ($did_something) {
        ...
        while ( my $def_ctx = $self->_defer_next ) {
            ...
            $instance->update( { deferred => undef } );
            $self->debug(
                "runner: _run drainafter " . $activity->activity_uid );
            $self->_enqueue_ai( $activity, $instance, 1 );
        }
...
sub _execute_implementation {
    ...
    if ( $activity->is_impl_subflow ) {
        ...
        foreach my $task ( $activity->tasks->all ) {
            ...
            $i++
                if ( $self->dryrun
                || $self->execute_task( $task, $instance ) );
            $j++;
        }
        $completed = $i == $j ? 1 : 0;
    }
...
sub complete_activity {
    ...
    if ( $activity->is_end_activity() ) {
        unless (
            $self->process_instance->activity_instances_rs->active->count )
        {
            $self->complete_process();
            return;
        }
...
sub _execute_transitions {
    ...
    if ( $fired_count == 0 ) {
        ...
        my $t_instance
            = $self->_execute_transition( $otherwise, $instance, 0 );
        if ($t_instance) {
...
sub _execute_transition {
    ...
    if ($err) {
        ...
        throw_runner error =>
            "Applying transition did not return an instance";
    }
