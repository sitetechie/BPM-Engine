...
if(00) {
    ...
    my $tAB  = $p->transitions->find({ transition_uid => 'A-B' });
    my $tAB1 = $p->transitions->find({ transition_uid => 'A-B1' });
    my $tBB1 = $p->transitions->find({ transition_uid => 'B-B1' });
    my $tBC  = $p->transitions->find({ transition_uid => 'B-C' });
    my $tB1D = $p->transitions->find({ transition_uid => 'B1-D' });
    my $tCD  = $p->transitions->find({ transition_uid => 'C-D' });

    my $aA  = $p->activities->find({ activity_uid => 'A' });
    my $aB  = $p->activities->find({ activity_uid => 'B' });
    my $aB1 = $p->activities->find({ activity_uid => 'B1' });
    my $aC  = $p->activities->find({ activity_uid => 'C' });
    my $aD  = $p->activities->find({ activity_uid => 'D' });
    ...
    is($ai_B1a->activity->activity_uid, 'B1','derive_and_accept results in B1');
...
# path B-C not taken yet
    ...
    is($ai_Da->activity->activity_uid, 'D','derive_and_accept results in D');
